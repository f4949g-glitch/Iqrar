-- عقود PDF أُرسلت للتوقيع دون رفع ملف فعليًا (original_file_path فارغ)، أو عقود
-- محرر أُرسلت دون محتوى (body_json فارغ)، كانت تصل حالة "مكتمل جزئيًا" عالقة إلى
-- الأبد بعد توقيع كل الأطراف، لأن التوثيق النهائي (finalizePdfContract/
-- finalizeEditorContract في submit-signature) يتوقف بصمت حين لا يجد محتوى ليعالجه.
-- الحل الجذري: منع إرسال عقد بلا محتوى فعلي من الأساس.
...
begin
  ...
  if v_row.document_type = 'power_of_attorney' then
    if v_party_count <> 1 then
      raise exception 'لا يمكن أن يتضمن التفويض أكثر من طرف واحد';
    end if;
