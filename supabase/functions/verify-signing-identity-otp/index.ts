...
Deno.serve(async (req: Request) => {
  ...
  const { data: party, error: partyError } = await admin.from('contract_parties').select('id').eq('token', token).maybeSingle();
  if (partyError || !party) return jsonResponse({ error: 'الرابط غير صالح أو منتهي' }, 404);

  const { data: otpRows } = await admin.rpc('rpc_get_signing_identity_otp', { p_party_id: party.id });
  const otp = otpRows?.[0];
  ...
  if (otp.code !== code) {
    await admin.rpc('rpc_increment_signing_identity_otp_attempts', { p_party_id: party.id });
    return jsonResponse({ error: 'رمز التحقق غير صحيح' }, 400);
  }

  await admin.rpc('rpc_mark_signing_identity_otp_verified', { p_party_id: party.id });
