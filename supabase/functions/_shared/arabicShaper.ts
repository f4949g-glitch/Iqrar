...
// (Arabic Presentation Forms) القابلة للرسم مباشرة بـ pdf-lib. ضروري لأن pdf-lib
// لا يطبّق قواعد ربط الحروف العربية (init/medi/fina) — انظر generateFinalPdf.ts.
//
// اتجاه النص: الناتج يبقى بالترتيب المنطقي (لا يُعكَس هنا) لأن محرّك fontkit
// الذي يستخدمه pdf-lib عند تضمين خط مخصص يكتشف أن السلسلة عربية ويعكسها
// تلقائيًا للاتجاه البصري عند الرسم. النسخة السابقة كانت تعكس يدويًا أيضًا،
// فيُعكَس النص مرتين ويظهر مقلوبًا في المستند النهائي ("صفحة التوثيق" كانت
// تظهر "قيثوتلا ةحفص") — أُثبت ذلك بتجربة رسم فعلية بنفس الخط والمكتبات.
//
// الأرقام المُضمَّنة داخل النص العربي (سنة، تاريخ 2026-07-22، وقت 09:32):
// عكس fontkit الشامل يقلب ترتيب خاناتها الداخلي، لذلك تُعكَس مقاطع الأرقام
// (مع فواصلها الداخلية - : .) مسبقًا هنا وهي بترتيبها المنطقي، فيعيدها عكسُ
// fontkit إلى ترتيبها الصحيح عند الرسم.
//
...
// يُنتج نصًا عربيًا عاديًا قابلاً لإعادة الاستخدام مباشرة. الحروف اللاتينية
// المُضمَّنة غير مدعومة وتُستبدَل بمسافة (خارج SAFE_OTHER).
...
export function reshapeArabicText(input: string): string {
  ...
  // لا عكس يدوي هنا (fontkit يعكس السلسلة العربية بالكامل تلقائيًا عند الرسم).
  // مقاطع الأرقام فقط تُعكَس مسبقًا (تبدأ وتنتهي برقم، وقد تحوي - : . داخلية
  // كالتواريخ والأوقات) كي يعيدها عكس fontkit الشامل إلى ترتيبها الصحيح.
  return out.join('').replace(/[0-9](?:[0-9:.-]*[0-9])?/g, (run) => Array.from(run).reverse().join(''));
}
