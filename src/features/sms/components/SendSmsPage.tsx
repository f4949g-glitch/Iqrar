...
import { sendAdminSms, fetchSmsHistory, type SmsMessage } from '../api/smsApi';
import { SmsTemplatesSection } from './SmsTemplatesSection';

const STATUS_BADGE: Record<SmsMessage['status'], { label: string; className: string }> = {
...
export function SendSmsPage() {
  ...
  const loadHistory = async () => {
    ...
    try {
      setHistory(await fetchSmsHistory());
      setHistoryError('');
    } catch (err) {
  ...
  const send = async () => {
    ...
    if (!phone || !message.trim()) {
      setError('رقم الجوال ونص الرسالة مطلوبان');
      return;
    }
    setSending(true);
    try {
      await sendAdminSms(phone, message.trim());
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-display text-2xl font-extrabold text-ink">إرسال رسائل SMS</h1>
          <p className="mt-1 text-sm text-slate">إرسال رسالة نصية مباشرة لأي رقم جوال عبر بوابة فور جوالي، مع سجل وقوالب الرسائل التلقائية.</p>
        </div>
        <div className="flex gap-1.5 rounded-lg bg-paper p-1">
          <button
            type="button"
            onClick={() => setTab('send')}
            className={`rounded-md px-3 py-1.5 text-xs font-bold transition ${tab === 'send' ? 'bg-card text-ink shadow-sm' : 'text-slate'}`}
          >
            إرسال وسجل
          </button>
          <button
            ...
            className={`rounded-md px-3 py-1.5 text-xs font-bold transition ${tab === 'templates' ? 'bg-card text-ink shadow-sm' : 'text-slate'}`}
          >
            قوالب الرسائل
          </button>
        </div>
          ...
          >
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              ...
              <label className="block text-sm sm:col-span-2">
                ...
                />
                <span className="mt-1 block text-xs text-slate">{message.length} / 900 حرفًا</span>
              </label>
            </div>
